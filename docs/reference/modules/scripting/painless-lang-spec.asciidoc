...
Ultimately, any left-hand side variable can be assigned to any right-hand side expression as long as the types match.
...
|Instance Of|instanceof|7|left-to-right
|Equality Equals|==|8|left-to-right
|====
|====
|Name|Symbol|Purpose(s)|
|Parenthesis|()|Precedence
|Equals Equals|==|Reference Equality
|Identity Equals|===|Identity Equality
|Not Equals|!=|Reference Inequality
|Identity Not Equals|!==|Identity Inequality
|Ampersand|&|Bitwise And
|Carrot|^|Boolean Xor & Bitwise Xor
|Pipe|\||Bitwise Or
|And|&&|Boolean And
|Or|\|\||Boolean Or
|Conditional|?|Conditional
|Elvis|?:|Conditional with Null Shortcut
...
A member may be accessed within a reference type using the dot '.' operator followed by the id of the member.  For primitive type members and reference type members, the accessed member behaves the same way as the type is represents with one exception; if the reference type is of type def then the member will also be considered to be of type def and resolved as such at run-time.  For method call members, see method calls [MARK].
...
Bitwise not will flip each bit of an integer value using the tilde operator.  The format is the tilde operator followed by an expression.
