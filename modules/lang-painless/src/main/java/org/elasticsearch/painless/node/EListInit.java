...
public final class EListInit extends AExpression {
    ...
    void analyze(Locals locals) {
        ...
        constructor = locals.getDefinition().getPainlessStruct(actual).constructors.get(new MethodKey("<init>", 0));

        if (constructor == null) {
            throw createError(new IllegalStateException("Illegal tree structure."));
        }

        method = locals.getDefinition().getPainlessStruct(actual).methods.get(new MethodKey("add", 1));
