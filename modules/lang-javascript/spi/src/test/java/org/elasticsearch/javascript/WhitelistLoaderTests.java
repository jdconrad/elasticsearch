...
public class WhitelistLoaderTests extends ESTestCase {
    ...
    public void testUnknownAnnotations() {
        ...
        RuntimeException expected = expectThrows(RuntimeException.class, () -> {
            WhitelistLoader.loadFromResourceFiles(Whitelist.class, parsers, "org.elasticsearch.javascript.annotation.unknown");
        });
        ...
        expected = expectThrows(RuntimeException.class, () -> {
            WhitelistLoader.loadFromResourceFiles(Whitelist.class, parsers, "org.elasticsearch.javascript.annotation.unknown_with_options");
        });
        assertEquals(
    ...
    public void testAnnotations() {
        ...
        Whitelist whitelist = WhitelistLoader.loadFromResourceFiles(Whitelist.class, parsers, "org.elasticsearch.javascript.annotation");
        ...
        for (WhitelistMethod whitelistMethod : whitelistClass.whitelistMethods) {
            if ("deprecatedMethod".equals(whitelistMethod.methodName)) {
                assertEquals(
                    "use another method",
                    ((DeprecatedAnnotation) whitelistMethod.javascriptAnnotations.get(DeprecatedAnnotation.class)).message()
                );
            ...
            if ("annotatedTestMethod".equals(whitelistMethod.methodName)) {
                AnnotationTestObject.TestAnnotation ta = ((AnnotationTestObject.TestAnnotation) whitelistMethod.javascriptAnnotations.get(
    ...
    public void testMissingWhitelistResource() {
        ...
        assertThat(
            e.getMessage(),
            equalTo(
                "Whitelist file [org/elasticsearch/javascript/spi/missing.txt] not found"
                    + " from owning class [org.elasticsearch.javascript.spi.Whitelist]."
            )
        );
    }
