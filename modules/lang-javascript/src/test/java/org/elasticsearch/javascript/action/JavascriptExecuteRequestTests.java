...
import org.elasticsearch.javascript.action.JavascriptExecuteAction.Request.ContextSetup;
...
public class JavascriptExecuteRequestTests extends AbstractWireSerializingTestCase<JavascriptExecuteAction.Request> {
    ...
    public final void testFromXContent() throws Exception {
        for (int i = 0; i < 20; i++) {
            JavascriptExecuteAction.Request testInstance = createTestInstance();
            ...
            try (XContentBuilder builder = XContentBuilder.builder(xContent)) {
                ...
                try (XContentParser parser = createParser(xContent, BytesReference.bytes(builder))) {
                    JavascriptExecuteAction.Request result = JavascriptExecuteAction.Request.parse(parser);
    ...
    @Override
    protected JavascriptExecuteAction.Request createTestInstance() {
        Script script = new Script(randomAlphaOfLength(10));
        ScriptContext<?> context = randomBoolean() ? randomFrom(JavascriptExecuteAction.Request.SUPPORTED_CONTEXTS.values()) : null;
        ContextSetup contextSetup = randomBoolean() ? randomContextSetup() : null;
        return new JavascriptExecuteAction.Request(script, context != null ? context.name : null, contextSetup);
    }

    @Override
    protected JavascriptExecuteAction.Request mutateInstance(JavascriptExecuteAction.Request instance) {
        return null;// TODO implement https://github.com/elastic/elasticsearch/issues/25929
    }

    @Override
    protected Writeable.Reader<JavascriptExecuteAction.Request> instanceReader() {
        return JavascriptExecuteAction.Request::new;
    }

    public void testValidate() {
        Script script = new Script(ScriptType.STORED, null, randomAlphaOfLength(10), Collections.emptyMap());
        JavascriptExecuteAction.Request request = new JavascriptExecuteAction.Request(script, null, null);
