...
public class DebugTests extends ScriptTestCase {
    ...
    public void testExplain() {
        ...
        assertThat(wrapper.realCause.getClass(), equalTo(JavascriptExplainError.class));
        var e = (JavascriptExplainError) wrapper.realCause;
        assertSame(dummy, e.getObjectToExplain());
        assertThat(e.getHeaders(javascriptLookup), hasEntry("es.to_string", singletonList(dummy.toString())));
        assertThat(e.getHeaders(javascriptLookup), hasEntry("es.java_class", singletonList("java.lang.Object")));
        assertThat(e.getHeaders(javascriptLookup), hasEntry("es.javascript_class", singletonList("java.lang.Object")));

        // Null should be ok
        wrapper = expectScriptThrows(ErrorCauseWrapper.class, () -> exec("Debug.explain(null)"));
        assertThat(wrapper.realCause.getClass(), equalTo(JavascriptExplainError.class));
        e = (JavascriptExplainError) wrapper.realCause;
        assertNull(e.getObjectToExplain());
        assertThat(e.getHeaders(javascriptLookup), hasEntry("es.to_string", singletonList("null")));
        assertThat(e.getHeaders(javascriptLookup), not(hasKey("es.java_class")));
        assertThat(e.getHeaders(javascriptLookup), not(hasKey("es.javascript_class")));
    ...
    /**
     * {@link JavascriptExplainError} doesn't serialize but the headers still make it.
     */
    public void testJavascriptExplainErrorSerialization() throws IOException {
        ...
        assertEquals(singletonList("java.lang.String"), e.getMetadata("es.javascript_class"));
        ...
        try (BytesStreamOutput out = new BytesStreamOutput()) {
            ...
            try (StreamInput in = out.bytes().streamInput()) {
                ...
                assertEquals(singletonList("java.lang.String"), read.getMetadata("es.javascript_class"));
            }
        }
    }
}
