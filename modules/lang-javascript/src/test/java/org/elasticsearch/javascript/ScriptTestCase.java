...
import org.elasticsearch.javascript.antlr.Walker;
...
public abstract class ScriptTestCase extends ESTestCase {
    ...
    protected static final List<Whitelist> JAVASCRIPT_BASE_WHITELIST = JavascriptPlugin.baseWhiteList();

    protected JavascriptScriptEngine scriptEngine;

    @Before
    public void setup() {
        scriptEngine = new JavascriptScriptEngine(scriptEngineSettings(), scriptContexts());
    }
    ...
    public Object exec(String script, Map<String, Object> vars, Map<String, String> compileParams, boolean picky) {
        ...
        if (picky) {
            ...
            Walker.buildJavascriptTree(getTestName(), script, pickySettings);
        }
        // test actual script execution
        JavascriptTestScript.Factory factory = scriptEngine.compile(null, script, JavascriptTestScript.CONTEXT, compileParams);
        JavascriptTestScript testScript = factory.newInstance(vars == null ? Collections.emptyMap() : vars);
