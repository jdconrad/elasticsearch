...
/**
 * JavascriptLookupUtility contains methods shared by {@link JavascriptLookupBuilder}, {@link JavascriptLookup}, and other classes within
 * Javascript for conversion between type names and types along with some other various utility methods.
 *
 ...
 *
 * Under ambiguous circumstances most variable names are prefixed with asm, java, or javascript. If the variable value is the same for asm,
 * java, and javascript, no prefix is used. Target is used as a prefix to represent if a constructor, method, or field is being
 ...
 *
 *     <li> - importedClassName (String)         - the imported javascript class name where the java canonical class name is used without
 *                                                 the package qualifier
 *
 *     <li> - canonicalClassName (String)        - the fully qualified javascript class name equivalent to the fully
 *                                                 qualified java canonical class name or imported javascript class name for a class
 *                                                 including def and excluding array types where '.' tokens represent inner classes <li>
 *
 *     <li> - canonicalTypeName (String)         - the fully qualified javascript type name equivalent to the fully
 *                                                 qualified java canonical type name or imported javascript type name for a type
 ...
 *
 *     <li> - class/clazz       (Class)          - a javascript class represented by a java class including def and excluding array
 *                                                 types </li>
 *
 *     <li> - type              (Class)          - a javascript type represented by a java class including def and array types </li>
 *
 *     <li> - javascriptClass     (JavascriptClass)  - a javascript class object </li>
 *
 *     <li> - javascriptMethod    (JavascriptMethod) - a javascript method object </li>
 *
 *     <li> - javascriptField     (JavascriptField)  - a javascript field object </li>
 * </ul>
 */
public final class JavascriptLookupUtility {
     ...
     * {@link JavascriptLookupUtility}. Since canonical class names are a subset of canonical type names, this method will
     ...
     * {@link JavascriptLookupUtility}. Since classes are a subset of types, this method will safely convert a class
     ...
     * documentation for {@link JavascriptLookupUtility}. Since classes are a subset of types, this method will safely convert a list
    ...
    public static String typesToCanonicalTypeNames(List<Class<?>> types) {
        return types.stream().map(JavascriptLookupUtility::typeToCanonicalTypeName).collect(Collectors.joining(",", "[", "]"));
    }

    /**
     * Converts a java type to a type based on the terminology specified as part of {@link JavascriptLookupUtility} where if a type is an
    ...
    /**
     * Converts a type to a java type based on the terminology specified as part of {@link JavascriptLookupUtility} where if a type is a
     ...
     * {@link JavascriptLookupUtility}. Otherwise, this behaves as an identity function.
     */
    ...
    /**
     * Checks if a type based on the terminology specified as part of {@link JavascriptLookupUtility} is available as a constant type
    ...
    /**
     * Constructs a javascript constructor key used to lookup javascript constructors from a javascript class.
     */
    public static String buildJavascriptConstructorKey(int constructorArity) {
        return CONSTRUCTOR_NAME + "/" + constructorArity;
    }

    /**
     * Constructs a javascript method key used to lookup javascript methods from a javascript class.
     */
    public static String buildJavascriptMethodKey(String methodName, int methodArity) {
        return methodName + "/" + methodArity;
    }

    /**
     * Constructs a javascript field key used to lookup javascript fields from a javascript class.
     */
    public static String buildJavascriptFieldKey(String fieldName) {
        return fieldName;
    }

    /**
     * Constructs an array of injectable constants for a specific {@link JavascriptMethod}
     * derived from an {@link org.elasticsearch.javascript.spi.annotation.InjectConstantAnnotation}.
     */
    public static Object[] buildInjections(JavascriptMethod javascriptMethod, Map<String, Object> constants) {
        InjectConstantAnnotation injects = (InjectConstantAnnotation) javascriptMethod.annotations().get(InjectConstantAnnotation.class);
        if (injects == null) {
            return new Object[0];
        }
            ...
            if (constant == null) {
                throw new IllegalStateException(
                    "constant ["
                        ...
                        + "["
                        + buildJavascriptMethodKey(javascriptMethod.javaMethod().getName(), javascriptMethod.typeParameters().size())
                        + "]"
                );
            }
    ...
    private JavascriptLookupUtility() {
