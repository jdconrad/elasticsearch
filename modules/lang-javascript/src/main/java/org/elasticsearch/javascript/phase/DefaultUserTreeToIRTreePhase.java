...
import org.elasticsearch.javascript.DefBootstrap;
import org.elasticsearch.javascript.FunctionRef;
import org.elasticsearch.javascript.Location;
import org.elasticsearch.javascript.MethodWriter;
...
import org.elasticsearch.javascript.ir.ConditionNode;
...
import org.elasticsearch.javascript.ir.UnaryNode;
...
import org.elasticsearch.javascript.symbol.Decorations.AccessDepth;
...
import org.elasticsearch.javascript.symbol.Decorations.Compound;
...
import org.elasticsearch.javascript.symbol.Decorations.IRNodeDecoration;
...
import org.elasticsearch.javascript.symbol.IRDecorations.IRCSynthetic;
import org.elasticsearch.javascript.symbol.IRDecorations.IRCVarArgs;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDArrayName;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDArrayType;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDBinaryType;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDCaptureNames;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDCast;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDClassBinding;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDComparisonType;
...
import org.elasticsearch.javascript.symbol.IRDecorations.IRDField;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDFieldType;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDFlags;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDFunction;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDIndexName;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDIndexType;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDIndexedType;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDInstanceBinding;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDInstanceType;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDIterableName;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDIterableType;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDMaxLoopCounter;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDMethod;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDModifiers;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDName;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDOperation;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDParameterNames;
...
import org.elasticsearch.javascript.symbol.IRDecorations.IRDSymbol;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDThisMethod;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDTypeParameters;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDUnaryType;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDValue;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDVariableName;
import org.elasticsearch.javascript.symbol.IRDecorations.IRDVariableType;
...
public class DefaultUserTreeToIRTreePhase implements UserTreeVisitor<ScriptScope> {
     ...
     * {@link DefBootstrap#bootstrap(JavascriptLookup, FunctionTable, Map, Lookup, String, MethodType, int, int, Object...)}
    ...
    protected void injectBootstrapMethod(ScriptScope scriptScope) {
        ...
        irFieldNode.attachDecoration(new IRDFieldType(JavascriptLookup.class));
        ...
        try {
            ...
            irLoadFieldMemberNode.attachDecoration(new IRDExpressionType(JavascriptLookup.class));
    ...
    protected ExpressionNode injectCast(AExpression userExpressionNode, ScriptScope scriptScope) {
        ...
        ExpressionJavascriptCast expressionJavascriptCast = scriptScope.getDecoration(userExpressionNode, ExpressionJavascriptCast.class);

        if (expressionJavascriptCast == null) {
            return irExpressionNode;
        }

        JavascriptCast javascriptCast = expressionJavascriptCast.expressionJavascriptCast();
        Class<?> targetType = javascriptCast.targetType;

        if (javascriptCast.boxTargetType != null) {
            targetType = JavascriptLookupUtility.typeToBoxedType(javascriptCast.boxTargetType);
        } else if (javascriptCast.unboxTargetType != null) {
            targetType = javascriptCast.unboxTargetType;
        }
        ...
        irCastNode.attachDecoration(new IRDCast(javascriptCast));
    ...
    public void visitEach(SEach userEachNode, ScriptScope scriptScope) {
        ...
        JavascriptCast javascriptCast = scriptScope.hasDecoration(userEachNode, ExpressionJavascriptCast.class)
            ? scriptScope.getDecoration(userEachNode, ExpressionJavascriptCast.class).expressionJavascriptCast()
            : null;
        ...
        if (iterableValueType.isArray()) {
            ...
            if (javascriptCast != null) {
                irForEachSubArrayNode.attachDecoration(new IRDCast(javascriptCast));
            }
            ...
            if (iterableValueType != def.class) {
                ...
                irForEachSubIterableNode.attachDecoration(
                    new IRDMethod(scriptScope.getDecoration(userEachNode, IterableJavascriptMethod.class).iterableJavascriptMethod())
                );

                if (javascriptCast != null) {
                    irForEachSubIterableNode.attachDecoration(new IRDCast(javascriptCast));
                }
            } else {
                ...
                if (javascriptCast != null && variable.type().isPrimitive() == false) {
                    irForEachSubIterableNode.attachDecoration(new IRDCast(javascriptCast));
                }
            }
    ...
    public void visitAssignment(EAssignment userAssignmentNode, ScriptScope scriptScope) {
        ...
        if (compoundType != null) {
            ...
            JavascriptCast downcast = scriptScope.hasDecoration(userAssignmentNode, DowncastJavascriptCast.class)
                ? scriptScope.getDecoration(userAssignmentNode, DowncastJavascriptCast.class).downcastJavascriptCast()
                : null;
            ...
            JavascriptCast upcast = scriptScope.hasDecoration(userAssignmentNode, UpcastJavascriptCast.class)
                ? scriptScope.getDecoration(userAssignmentNode, UpcastJavascriptCast.class).upcastJavascriptCast()
                : null;
    ...
    public void visitNewObj(ENewObj userNewObjectNode, ScriptScope scriptScope) {
        ...
        JavascriptConstructor javascriptConstructor = scriptScope.getDecoration(userNewObjectNode, StandardJavascriptConstructor.class)
            .standardJavascriptConstructor();
        ...
        irNewObjectNode.attachDecoration(new IRDConstructor(javascriptConstructor));
    ...
    public void visitCallLocal(ECallLocal callLocalNode, ScriptScope scriptScope) {
        ...
        if (scriptScope.hasDecoration(callLocalNode, StandardLocalFunction.class)) {
        ...
        } else if (scriptScope.hasDecoration(callLocalNode, ThisJavascriptMethod.class)) {
            ...
            irFieldNode.attachDecoration(new IRDFieldType(javascriptClassBinding.javaConstructor().getDeclaringClass()));
            irFieldNode.attachDecoration(new IRDName(bindingName));
            irClassNode.addFieldNode(irFieldNode);

            irInvokeCallMemberNode.attachDecoration(new IRDClassBinding(javascriptClassBinding));
        ...
        } else if (scriptScope.hasDecoration(callLocalNode, StandardJavascriptInstanceBinding.class)) {
            JavascriptInstanceBinding javascriptInstanceBinding = scriptScope.getDecoration(
                callLocalNode,
                StandardJavascriptInstanceBinding.class
            ).javascriptInstanceBinding();
            ...
            irFieldNode.attachDecoration(new IRDFieldType(javascriptInstanceBinding.targetInstance().getClass()));
            irFieldNode.attachDecoration(new IRDName(bindingName));
            irClassNode.addFieldNode(irFieldNode);

            irInvokeCallMemberNode.attachDecoration(new IRDInstanceBinding(javascriptInstanceBinding));
            irInvokeCallMemberNode.attachDecoration(new IRDName(bindingName));

            scriptScope.addStaticConstant(bindingName, javascriptInstanceBinding.targetInstance());
        } else {
        ...
        if (scriptScope.hasDecoration(userDotNode, StaticType.class)) {
            ...
            if (prefixValueType != null && prefixValueType.valueType().isArray()) {
                ...
                if (write || compound) {
                    ...
                    irStoreDotNode.attachDecoration(new IRDField(javascriptField));
                ...
                if (write == false || compound) {
                    ...
                    irLoadDotNode.attachDecoration(new IRDField(javascriptField));
                ...
                if (write || compound) {
                    ...
                    irStoreDotShortcutNode.attachDecoration(
                        new IRDMethod(scriptScope.getDecoration(userDotNode, SetterJavascriptMethod.class).setterJavascriptMethod())
                    );
                ...
                if (write == false || compound) {
                    ...
                    irLoadDotShortcutNode.attachDecoration(
                        new IRDMethod(scriptScope.getDecoration(userDotNode, GetterJavascriptMethod.class).getterJavascriptMethod())
                    );
    ...
    public void visitBrace(EBrace userBraceNode, ScriptScope scriptScope) {
        ...
        if (prefixValueType.isArray()) {
            ...
            if (write || compound) {
                JavascriptMethod setter = scriptScope.getDecoration(userBraceNode, SetterJavascriptMethod.class).setterJavascriptMethod();
                StoreMapShortcutNode irStoreMapShortcutNode = new StoreMapShortcutNode(location);
                irStoreMapShortcutNode.attachDecoration(new IRDExpressionType(read ? valueType : void.class));
                irStoreMapShortcutNode.attachDecoration(new IRDStoreType(valueType));
            ...
            if (write == false || compound) {
                JavascriptMethod getter = scriptScope.getDecoration(userBraceNode, GetterJavascriptMethod.class).getterJavascriptMethod();
                LoadMapShortcutNode irLoadMapShortcutNode = new LoadMapShortcutNode(location);
                irLoadMapShortcutNode.attachDecoration(new IRDExpressionType(valueType));
    ...
    public void visitCall(ECall userCallNode, ScriptScope scriptScope) {
        ...
        if (scriptScope.getCondition(userCallNode, DynamicInvocation.class)) {
            ...
            JavascriptMethod method = scriptScope.getDecoration(userCallNode, StandardJavascriptMethod.class).standardJavascriptMethod();
            Object[] injections = JavascriptLookupUtility.buildInjections(method, scriptScope.getCompilerSettings().asMap());
            ...
            for (int i = 0; i < injections.length; i++) {
                ...
                if (parameterType != JavascriptLookupUtility.typeToUnboxedType(injection.getClass())) {
            ...
            irInvokeCallNode.setMethod(scriptScope.getDecoration(userCallNode, StandardJavascriptMethod.class).standardJavascriptMethod());
