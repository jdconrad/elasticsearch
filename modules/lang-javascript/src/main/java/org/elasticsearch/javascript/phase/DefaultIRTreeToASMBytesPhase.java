...
import org.elasticsearch.javascript.ClassWriter;
...
import org.elasticsearch.javascript.WriterConstants;
import org.elasticsearch.javascript.api.Augmentation;
...
import org.elasticsearch.javascript.symbol.WriteScope.Variable;
...
import static org.elasticsearch.javascript.WriterConstants.EQUALS;
import static org.elasticsearch.javascript.WriterConstants.ITERATOR_HASNEXT;
import static org.elasticsearch.javascript.WriterConstants.ITERATOR_NEXT;
import static org.elasticsearch.javascript.WriterConstants.ITERATOR_TYPE;
import static org.elasticsearch.javascript.WriterConstants.OBJECTS_TYPE;
import static org.elasticsearch.javascript.WriterConstants.VALUE_ITERATOR_NEXT_BOOLEAN;
import static org.elasticsearch.javascript.WriterConstants.VALUE_ITERATOR_NEXT_BYTE;
import static org.elasticsearch.javascript.WriterConstants.VALUE_ITERATOR_NEXT_CHAR;
import static org.elasticsearch.javascript.WriterConstants.VALUE_ITERATOR_NEXT_DOUBLE;
import static org.elasticsearch.javascript.WriterConstants.VALUE_ITERATOR_NEXT_FLOAT;
import static org.elasticsearch.javascript.WriterConstants.VALUE_ITERATOR_NEXT_INT;
import static org.elasticsearch.javascript.WriterConstants.VALUE_ITERATOR_NEXT_LONG;
import static org.elasticsearch.javascript.WriterConstants.VALUE_ITERATOR_NEXT_SHORT;
import static org.elasticsearch.javascript.WriterConstants.VALUE_ITERATOR_TYPE;
...
public class DefaultIRTreeToASMBytesPhase implements IRTreeVisitor<WriteScope> {
    ...
    public void visitForEachSubIterableLoop(ForEachSubIterableNode irForEachSubIterableNode, WriteScope writeScope) {
        ...
        JavascriptMethod javascriptMethod = irForEachSubIterableNode.getDecorationValue(IRDMethod.class);
        ...
        if (javascriptMethod == null) {
            ...
            methodWriter.invokeMethodCall(javascriptMethod);
        }
        ...
        methodWriter.visitVarInsn(iterator.getAsmType().getOpcode(Opcodes.ILOAD), iterator.getSlot());
        if (javascriptMethod != null || variable.getType().isPrimitive() == false) {
    ...
    public void visitInstanceof(InstanceofNode irInstanceofNode, WriteScope writeScope) {
        ...
        if (instanceType == def.class) {
            ...
            Class<?> boxedInstanceType = JavascriptLookupUtility.typeToBoxedType(instanceType);
            Class<?> childExpressionType = irChildNode.getDecorationValue(IRDExpressionType.class);
            Class<?> boxedExpressionType = JavascriptLookupUtility.typeToBoxedType(childExpressionType);

            methodWriter.writePop(MethodWriter.getType(expressionType).getSize());
            methodWriter.push(boxedInstanceType.isAssignableFrom(boxedExpressionType));
        } else {
            methodWriter.instanceOf(MethodWriter.getType(JavascriptLookupUtility.typeToBoxedType(instanceType)));
        }
    }
    ...
    public void visitListInitialization(ListInitializationNode irListInitializationNode, WriteScope writeScope) {
        ...
        JavascriptConstructor javascriptConstructor = irListInitializationNode.getDecorationValue(IRDConstructor.class);
        ...
        methodWriter.invokeConstructor(
            Type.getType(javascriptConstructor.javaConstructor().getDeclaringClass()),
            Method.getMethod(javascriptConstructor.javaConstructor())
        );
    ...
    public void visitMapInitialization(MapInitializationNode irMapInitializationNode, WriteScope writeScope) {
        ...
        JavascriptConstructor javascriptConstructor = irMapInitializationNode.getDecorationValue(IRDConstructor.class);
    ...
    public void visitNewObject(NewObjectNode irNewObjectNode, WriteScope writeScope) {
        ...
        JavascriptConstructor javascriptConstructor = irNewObjectNode.getDecorationValue(IRDConstructor.class);
        methodWriter.invokeConstructor(
            Type.getType(javascriptConstructor.javaConstructor().getDeclaringClass()),
            Method.getMethod(javascriptConstructor.javaConstructor())
        );
    ...
    public void visitLoadDot(LoadDotNode irLoadDotNode, WriteScope writeScope) {
        ...
        JavascriptField javascriptField = irLoadDotNode.getDecorationValue(IRDField.class);
        boolean isStatic = Modifier.isStatic(javascriptField.javaField().getModifiers());
        Type asmOwnerType = Type.getType(javascriptField.javaField().getDeclaringClass());
        String fieldName = javascriptField.javaField().getName();
        Type asmFieldType = MethodWriter.getType(javascriptField.typeParameter());
    ...
    public void visitLoadDotShortcut(LoadDotShortcutNode irDotSubShortcutNode, WriteScope writeScope) {
        ...
        JavascriptMethod getterJavascriptMethod = irDotSubShortcutNode.getDecorationValue(IRDMethod.class);
        methodWriter.invokeMethodCall(getterJavascriptMethod);

        if (getterJavascriptMethod.returnType() != getterJavascriptMethod.javaMethod().getReturnType()) {
            methodWriter.checkCast(MethodWriter.getType(getterJavascriptMethod.returnType()));
        }
    }
    ...
    public void visitLoadListShortcut(LoadListShortcutNode irLoadListShortcutNode, WriteScope writeScope) {
        ...
        JavascriptMethod getterJavascriptMethod = irLoadListShortcutNode.getDecorationValue(IRDMethod.class);
        methodWriter.invokeMethodCall(getterJavascriptMethod);
    ...
    public void visitLoadMapShortcut(LoadMapShortcutNode irLoadMapShortcutNode, WriteScope writeScope) {
        ...
        JavascriptMethod getterJavascriptMethod = irLoadMapShortcutNode.getDecorationValue(IRDMethod.class);
        methodWriter.invokeMethodCall(getterJavascriptMethod);
    ...
    public void visitStoreDot(StoreDotNode irStoreDotNode, WriteScope writeScope) {
        ...
        JavascriptField javascriptField = irStoreDotNode.getDecorationValue(IRDField.class);
        boolean isStatic = Modifier.isStatic(javascriptField.javaField().getModifiers());
        Type asmOwnerType = Type.getType(javascriptField.javaField().getDeclaringClass());
        String fieldName = javascriptField.javaField().getName();
        Type asmFieldType = MethodWriter.getType(javascriptField.typeParameter());
    ...
    public void visitInvokeCallMember(InvokeCallMemberNode irInvokeCallMemberNode, WriteScope writeScope) {
        ...
        JavascriptMethod thisMethod = irInvokeCallMemberNode.getDecorationValue(IRDThisMethod.class);
        JavascriptMethod importedMethod = irInvokeCallMemberNode.getDecorationValue(IRDMethod.class);
        JavascriptClassBinding classBinding = irInvokeCallMemberNode.getDecorationValue(IRDClassBinding.class);
        JavascriptInstanceBinding instanceBinding = irInvokeCallMemberNode.getDecorationValue(IRDInstanceBinding.class);
