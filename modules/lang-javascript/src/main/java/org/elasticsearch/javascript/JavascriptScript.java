...
/**
 * Abstract superclass on top of which all Javascript scripts are built.
 */
public interface JavascriptScript {

    /**
     * @return The name of the script retrieved from a static variable generated
     * during compilation of a Javascript script.
     */
    String getName();
    ...
    default ScriptException convertToScriptException(Throwable t, Map<String, List<String>> extraMetadata) {
        if (t instanceof JavascriptWrappedException) {
            t = t.getCause();
        }
    ...
    default boolean shouldFilter(StackTraceElement element) {
        return element.getClassName().startsWith("org.elasticsearch.javascript.")
