...
public class JavascriptSemanticAnalysisPhase extends DefaultSemanticAnalysisPhase {
    ...
    public static void decorateWithCastForReturn(
        ...
        if (valueType == def.class) {
            ...
            for (LocalFunction converter : scriptClassInfo.converters) {
                try {
                    javascriptCast = AnalyzerCaster.getLegalCast(location, valueType, converter.getTypeParameters().get(0), false, true);
        ...
        javascriptCast = AnalyzerCaster.getLegalCast(location, valueType, targetType, isExplicitCast, isInternalCast);
        if (javascriptCast != null) {
            semanticScope.putDecoration(userExpressionNode, new ExpressionJavascriptCast(javascriptCast));
        }
    }
    ...
    public void visitCallLocal(ECallLocal userCallLocalNode, SemanticScope semanticScope) {
        if ("$".equals(userCallLocalNode.getMethodName())) {
            ...
            JavascriptMethod thisMethod = scriptScope.getJavascriptLookup()
                .lookupJavascriptMethod(scriptScope.getScriptClassInfo().getBaseClass(), false, "field", 1);
