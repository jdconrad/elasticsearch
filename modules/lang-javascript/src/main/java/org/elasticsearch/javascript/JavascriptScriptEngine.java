...
import org.elasticsearch.javascript.Compiler.Loader;
...
import static org.elasticsearch.javascript.WriterConstants.OBJECT_TYPE;

/**
 * Implementation of a ScriptEngine for the Javascript language.
 */
public final class JavascriptScriptEngine implements ScriptEngine {

    /**
     * Standard name of the Javascript language.
     */
     ...
     * of {@link JavascriptScriptEngine#JavascriptScriptEngine(Settings, Map)}.
     */
    ...
    private final Map<ScriptContext<?>, JavascriptLookup> contextsToLookups;

    /**
     * Constructor.
     * @param settings The settings to initialize the engine with.
     */
    public JavascriptScriptEngine(Settings settings, Map<ScriptContext<?>, List<Whitelist>> contexts) {
        ...
        Map<ScriptContext<?>, JavascriptLookup> mutableContextsToLookups = new HashMap<>();

        final Map<Object, Object> dedup = new HashMap<>();
        final Map<JavascriptMethod, JavascriptMethod> filteredMethodCache = new HashMap<>();
        for (Map.Entry<ScriptContext<?>, List<Whitelist>> entry : contexts.entrySet()) {
            ScriptContext<?> context = entry.getKey();
            JavascriptLookup lookup = JavascriptLookupBuilder.buildFromWhitelists(entry.getValue(), dedup, filteredMethodCache);
    ...
    public Map<ScriptContext<?>, JavascriptLookup> getContextsToLookups() {
    ...
    ScriptScope compile(Compiler compiler, Loader loader, String scriptName, String source, Map<String, String> params) {
        ...
        try {
            ...
            // Note that it is safe to catch any of the following errors since Javascript is stateless.
    ...
    private CompilerSettings buildCompilerSettings(Map<String, String> params) {
        ...
        if (params.isEmpty()) {
            ...
            if (value != null) {
                throw new IllegalArgumentException("[javascript.regex.enabled] can only be set on node startup.");
            }

            value = copy.remove(CompilerSettings.REGEX_LIMIT_FACTOR.getKey());
            if (value != null) {
                throw new IllegalArgumentException("[javascript.regex.limit-factor] can only be set on node startup.");
            }
    ...
    private static ScriptException convertToScriptException(String scriptSource, Throwable t) {
        ...
        throw new ScriptException("compile error", cause, scriptStack, scriptSource, JavascriptScriptEngine.NAME, pos);
    }
