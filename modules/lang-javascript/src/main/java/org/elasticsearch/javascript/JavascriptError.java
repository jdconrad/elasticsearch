...
/**
 * The JavascriptError class is used to throw internal errors caused by Javascript scripts that cannot be
 * caught using a standard {@link Exception}.  This prevents the user from catching this specific error
 * (as Exceptions are available in the Javascript API, but Errors are not,) and possibly continuing to do
...
public class JavascriptError extends Error {
     ...
     */
    public JavascriptError(final String message) {
        super(message);
    }
}
