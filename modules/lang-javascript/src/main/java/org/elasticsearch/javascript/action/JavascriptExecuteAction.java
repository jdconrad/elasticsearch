...
public class JavascriptExecuteAction {

    public static final ActionType<Response> INSTANCE = new ActionType<>("cluster:admin/scripts/javascript/execute");
    public static final RemoteClusterActionType<Response> REMOTE_TYPE = new RemoteClusterActionType<>(INSTANCE.name(), Response::new);

    private JavascriptExecuteAction() {/* no instances */}
    ...
    public static class Request extends SingleShardRequest<Request> implements ToXContentObject, IndicesRequest.SingleIndexNoWildcards {
        ...
        private static Map<String, ScriptContext<?>> getSupportedContexts() {
            ...
            contexts.put(JavascriptTestScript.CONTEXT.name, JavascriptTestScript.CONTEXT);
        ...
        static class ContextSetup implements Writeable, ToXContentObject {
            ...
            static Tuple<String, String> parseClusterAliasAndIndex(String indexExpression) {
                ...
                // because (1) only a single index is allowed for Javascript Execute and
        ...
        Request(Script script, String scriptContextName, ContextSetup setup) {
            ...
            this.context = scriptContextName != null ? fromScriptContextName(scriptContextName) : JavascriptTestScript.CONTEXT;
            if (setup != null) {
        ...
        public void markOriginOnly() {
            assert contextSetup != null
                : "Javascript/execute request without context setup can't have index, this method shouldn't be called";
    ...
    public static class TransportAction extends TransportSingleShardAction<Request, Response> {
        ...
        protected void doExecute(Task task, Request request, ActionListener<Response> listener) {
            ...
            if (isLocalIndex(request)) {
                ...
                transportService.getRemoteClusterService()
                    ...
                    .execute(JavascriptExecuteAction.REMOTE_TYPE, request, listener);
            }
        }
        ...
        static Response innerShardOperation(Request request, ScriptService scriptService, IndexService indexService) throws IOException {
            ...
            if (scriptContext == JavascriptTestScript.CONTEXT) {
                JavascriptTestScript.Factory factory = scriptService.compile(request.script, JavascriptTestScript.CONTEXT);
                JavascriptTestScript javascriptTestScript = factory.newInstance(request.script.getParams());
                String result = Objects.toString(javascriptTestScript.execute());
        ...
        private static Response prepareRamIndex(
            ...
            try (Directory directory = new ByteBuffersDirectory()) {
                try (IndexWriter indexWriter = new IndexWriter(directory, new IndexWriterConfig(defaultAnalyzer))) {
                    ...
                    // only index the root doc since nested docs are not supported in javascript anyways
    ...
    public static class RestAction extends BaseRestHandler {
        ...
        public List<Route> routes() {
            return List.of(new Route(GET, "/_scripts/javascript/_execute"), new Route(POST, "/_scripts/javascript/_execute"));
        }

        @Override
        public String getName() {
            return "_scripts_javascript_execute";
        }
