...
import static org.elasticsearch.javascript.WriterConstants.CLASS_NAME;
...
public class FunctionRef {
     ...
     * @param javascriptLookup the whitelist against which this script is being compiled
    ...
    public static FunctionRef create(
        ...
        String targetClassName = JavascriptLookupUtility.typeToCanonicalTypeName(targetClass);
        JavascriptMethod interfaceMethod;

        try {
            interfaceMethod = javascriptLookup.lookupFunctionalInterfaceJavascriptMethod(targetClass);
            ...
            if ("this".equals(typeName)) {
                ...
                delegateClassName = javascriptConstructor.javaConstructor().getDeclaringClass().getName();
                isDelegateInterface = false;
                isDelegateAugmented = false;
                delegateInvokeType = H_NEWINVOKESPECIAL;
                delegateMethodName = JavascriptLookupUtility.CONSTRUCTOR_NAME;
                delegateMethodType = javascriptConstructor.methodType();
                delegateInjections = new Object[0];

                delegateMethodReturnType = javascriptConstructor.javaConstructor().getDeclaringClass();
                delegateMethodParameters = javascriptConstructor.typeParameters();
            } else {
                ...
                JavascriptMethod javascriptMethod = javascriptLookup.lookupJavascriptMethod(
                    typeName,
                    true,
                    methodName,
                    interfaceTypeParametersSize
                ...
                if (javascriptMethod == null) {
                    javascriptMethod = javascriptLookup.lookupJavascriptMethod(
                        typeName,
                        false,
                        methodName,
                ...
                delegateClassName = javascriptMethod.javaMethod().getDeclaringClass().getName();
                isDelegateInterface = javascriptMethod.javaMethod().getDeclaringClass().isInterface();
                isDelegateAugmented = javascriptMethod.javaMethod().getDeclaringClass() != javascriptMethod.targetClass();
                ...
                delegateMethodName = javascriptMethod.javaMethod().getName();
                delegateMethodType = javascriptMethod.methodType();
                ...
                if (delegateInvokeType != H_INVOKESTATIC
                    && javascriptMethod.javaMethod().getDeclaringClass() != javascriptMethod.methodType().parameterType(0)) {
                    if (javascriptMethod.methodType().parameterType(0) != Object.class) {
                        throw new IllegalStateException("internal error");
                    }

                    delegateMethodType = delegateMethodType.changeParameterType(0, javascriptMethod.javaMethod().getDeclaringClass());
                }

                delegateInjections = JavascriptLookupUtility.buildInjections(javascriptMethod, constants);

                delegateMethodReturnType = javascriptMethod.returnType();

                if (delegateMethodType.parameterList().size() > javascriptMethod.typeParameters().size()) {
                    delegateMethodParameters = new ArrayList<>(javascriptMethod.typeParameters());
                    delegateMethodParameters.add(0, delegateMethodType.parameterType(0));
                } else {
                    delegateMethodParameters = javascriptMethod.typeParameters();
                }
            }
