...
import org.elasticsearch.javascript.AnalyzerCaster;
...
import org.elasticsearch.javascript.spi.annotation.DynamicTypeAnnotation;
import org.elasticsearch.javascript.spi.annotation.NonDeterministicAnnotation;
import org.elasticsearch.javascript.symbol.Decorations;
import org.elasticsearch.javascript.symbol.Decorations.AllEscape;
import org.elasticsearch.javascript.symbol.Decorations.AnyBreak;
import org.elasticsearch.javascript.symbol.Decorations.AnyContinue;
import org.elasticsearch.javascript.symbol.Decorations.BeginLoop;
import org.elasticsearch.javascript.symbol.Decorations.BinaryType;
import org.elasticsearch.javascript.symbol.Decorations.CaptureBox;
import org.elasticsearch.javascript.symbol.Decorations.CapturesDecoration;
import org.elasticsearch.javascript.symbol.Decorations.ComparisonType;
import org.elasticsearch.javascript.symbol.Decorations.CompoundType;
import org.elasticsearch.javascript.symbol.Decorations.ContinuousLoop;
import org.elasticsearch.javascript.symbol.Decorations.DefOptimized;
import org.elasticsearch.javascript.symbol.Decorations.DowncastJavascriptCast;
import org.elasticsearch.javascript.symbol.Decorations.DynamicInvocation;
import org.elasticsearch.javascript.symbol.Decorations.EncodingDecoration;
import org.elasticsearch.javascript.symbol.Decorations.Explicit;
import org.elasticsearch.javascript.symbol.Decorations.ExpressionJavascriptCast;
import org.elasticsearch.javascript.symbol.Decorations.GetterJavascriptMethod;
import org.elasticsearch.javascript.symbol.Decorations.InLoop;
import org.elasticsearch.javascript.symbol.Decorations.InstanceCapturingFunctionRef;
import org.elasticsearch.javascript.symbol.Decorations.InstanceCapturingLambda;
import org.elasticsearch.javascript.symbol.Decorations.InstanceType;
import org.elasticsearch.javascript.symbol.Decorations.Internal;
import org.elasticsearch.javascript.symbol.Decorations.IterableJavascriptMethod;
import org.elasticsearch.javascript.symbol.Decorations.LastLoop;
import org.elasticsearch.javascript.symbol.Decorations.LastSource;
import org.elasticsearch.javascript.symbol.Decorations.ListShortcut;
import org.elasticsearch.javascript.symbol.Decorations.LoopEscape;
import org.elasticsearch.javascript.symbol.Decorations.MapShortcut;
import org.elasticsearch.javascript.symbol.Decorations.MethodEscape;
import org.elasticsearch.javascript.symbol.Decorations.MethodNameDecoration;
import org.elasticsearch.javascript.symbol.Decorations.Negate;
import org.elasticsearch.javascript.symbol.Decorations.ParameterNames;
import org.elasticsearch.javascript.symbol.Decorations.PartialCanonicalTypeName;
import org.elasticsearch.javascript.symbol.Decorations.Read;
import org.elasticsearch.javascript.symbol.Decorations.ReferenceDecoration;
import org.elasticsearch.javascript.symbol.Decorations.ReturnType;
import org.elasticsearch.javascript.symbol.Decorations.SemanticVariable;
import org.elasticsearch.javascript.symbol.Decorations.SetterJavascriptMethod;
...
import org.elasticsearch.javascript.symbol.Decorations.UpcastJavascriptCast;
import org.elasticsearch.javascript.symbol.Decorations.ValueType;
import org.elasticsearch.javascript.symbol.Decorations.Write;
...
import org.elasticsearch.javascript.symbol.SemanticScope;
import org.elasticsearch.javascript.symbol.SemanticScope.FunctionScope;
import org.elasticsearch.javascript.symbol.SemanticScope.LambdaScope;
...
import static org.elasticsearch.javascript.symbol.SemanticScope.newFunctionScope;
...
public class DefaultSemanticAnalysisPhase extends UserTreeBaseVisitor<SemanticScope> {
    ...
    /**
     * Decorates a user expression node with a JavascriptCast.
     */
    public static void decorateWithCast(AExpression userExpressionNode, SemanticScope semanticScope) {
        ...
        JavascriptCast javascriptCast = AnalyzerCaster.getLegalCast(location, valueType, targetType, isExplicitCast, isInternalCast);

        if (javascriptCast != null) {
            semanticScope.putDecoration(userExpressionNode, new ExpressionJavascriptCast(javascriptCast));
        }
    }
    ...
    public void visitEach(SEach userEachNode, SemanticScope semanticScope) {
        ...
        Class<?> type = semanticScope.getScriptScope().getJavascriptLookup().canonicalTypeNameToType(canonicalTypeName);
        ...
        if (iterableValueType.isArray()) {
            JavascriptCast javascriptCast = AnalyzerCaster.getLegalCast(
                location,
            ...
            if (javascriptCast != null) {
                semanticScope.putDecoration(userEachNode, new ExpressionJavascriptCast(javascriptCast));
            }
        } else if (iterableValueType == def.class || Iterable.class.isAssignableFrom(iterableValueType)) {
            if (iterableValueType != def.class) {
                JavascriptMethod method = semanticScope.getScriptScope()
                    .getJavascriptLookup()
                    .lookupJavascriptMethod(iterableValueType, false, "iterator", 0);
                ...
                semanticScope.putDecoration(userEachNode, new IterableJavascriptMethod(method));
            }

            JavascriptCast javascriptCast = AnalyzerCaster.getLegalCast(location, def.class, type, true, true);
            ...
            throw userEachNode.createError(
                new IllegalArgumentException(
                    "invalid foreach loop: "
                        ...
                        + JavascriptLookupUtility.typeToCanonicalTypeName(iterableValueType)
                        + "]."
                )
            );
        }
    ...
    public void visitDeclaration(SDeclaration userDeclarationNode, SemanticScope semanticScope) {
        ...
        if (scriptScope.getJavascriptLookup().isValidCanonicalClassName(symbol)) {
            ...
            if (semanticScope.getReturnType() != void.class) {
                throw userReturnNode.createError(
                    castError(
                        ...
                        JavascriptLookupUtility.typeToCanonicalTypeName(void.class)
                    )
                );
            }
        } else {
    ...
    public void visitCatch(SCatch userCatchNode, SemanticScope semanticScope) {
        ...
        if (userCatchNode.getBaseException().isAssignableFrom(type) == false) {
            throw userCatchNode.createError(
                castError(
                    "cannot cast from [%s] to [%s]",
                    JavascriptLookupUtility.typeToCanonicalTypeName(type),
                    JavascriptLookupUtility.typeToCanonicalTypeName(baseException)
                )
    ...
    public void visitAssignment(EAssignment userAssignmentNode, SemanticScope semanticScope) {
        ...
        if (operation != null) {
            ...
            JavascriptCast upcast = AnalyzerCaster.getLegalCast(location, leftValueType, compoundType, false, false);
            JavascriptCast downcast = AnalyzerCaster.getLegalCast(location, compoundType, leftValueType, true, false);

            semanticScope.putDecoration(userAssignmentNode, new CompoundType(compoundType));

            if (upcast != null) {
                semanticScope.putDecoration(userAssignmentNode, new UpcastJavascriptCast(upcast));
            }

            if (downcast != null) {
                semanticScope.putDecoration(userAssignmentNode, new DowncastJavascriptCast(downcast));
            }
            ...
            if (rightValueType == void.class) {
                throw userAssignmentNode.createError(
                    new IllegalArgumentException(
                        "invalid assignment: cannot assign type [" + JavascriptLookupUtility.typeToCanonicalTypeName(void.class) + "]"
                    )
                );
            }
                ...
                if (unaryType == null) {
                    throw userUnaryNode.createError(
                        castError(
                            ...
                            operation.name,
                            operation.symbol,
                            JavascriptLookupUtility.typeToCanonicalTypeName(childValueType)
                        )
            ...
            if (binaryType == null) {
                throw userBinaryNode.createError(
                    castError(
                        ...
                        operation.name,
                        operation.symbol,
                        JavascriptLookupUtility.typeToCanonicalTypeName(leftValueType),
                        JavascriptLookupUtility.typeToCanonicalTypeName(rightValueType)
                    )
    ...
    public void visitExplicit(EExplicit userExplicitNode, SemanticScope semanticScope) {
        ...
        Class<?> valueType = semanticScope.getScriptScope().getJavascriptLookup().canonicalTypeNameToType(canonicalTypeName);
    ...
    public void visitInstanceof(EInstanceof userInstanceofNode, SemanticScope semanticScope) {
        ...
        Class<?> instanceType = semanticScope.getScriptScope().getJavascriptLookup().canonicalTypeNameToType(canonicalTypeName);
    ...
    public void visitListInit(EListInit userListInitNode, SemanticScope semanticScope) {
        ...
        JavascriptConstructor constructor = semanticScope.getScriptScope().getJavascriptLookup().lookupJavascriptConstructor(valueType, 0);
        ...
        semanticScope.putDecoration(userListInitNode, new StandardJavascriptConstructor(constructor));

        JavascriptMethod method = semanticScope.getScriptScope().getJavascriptLookup().lookupJavascriptMethod(valueType, false, "add", 1);
        ...
        semanticScope.putDecoration(userListInitNode, new StandardJavascriptMethod(method));
    ...
    public void visitMapInit(EMapInit userMapInitNode, SemanticScope semanticScope) {
        ...
        semanticScope.putDecoration(userMapInitNode, new StandardJavascriptConstructor(constructor));

        JavascriptMethod method = semanticScope.getScriptScope().getJavascriptLookup().lookupJavascriptMethod(valueType, false, "put", 2);
        ...
        semanticScope.putDecoration(userMapInitNode, new StandardJavascriptMethod(method));
    ...
    public void visitNewObj(ENewObj userNewObjNode, SemanticScope semanticScope) {
        ...
        ScriptScope scriptScope = semanticScope.getScriptScope();
        Class<?> valueType = scriptScope.getJavascriptLookup().canonicalTypeNameToType(canonicalTypeName);

        if (valueType == null) {
            throw userNewObjNode.createError(new IllegalArgumentException("Not a type [" + canonicalTypeName + "]."));
        }

        JavascriptConstructor constructor = scriptScope.getJavascriptLookup().lookupJavascriptConstructor(valueType, userArgumentsSize);
        ...
        scriptScope.putDecoration(userNewObjNode, new StandardJavascriptConstructor(constructor));
            ...
            throw userNewObjNode.createError(
                new IllegalArgumentException(
                    Strings.format(
                        ...
                        JavascriptLookupUtility.typeToCanonicalTypeName(valueType),
    ...
    public void visitCallLocal(ECallLocal userCallLocalNode, SemanticScope semanticScope) {
        ...
        JavascriptMethod thisMethod = null;
        JavascriptMethod importedMethod = null;
        ...
        if (localFunction == null) {
            thisMethod = scriptScope.getJavascriptLookup()
                .lookupJavascriptMethod(scriptScope.getScriptClassInfo().getBaseClass(), false, methodName, userArgumentsSize);

            if (thisMethod == null) {
                importedMethod = scriptScope.getJavascriptLookup().lookupImportedJavascriptMethod(methodName, userArgumentsSize);

                if (importedMethod == null) {
                    classBinding = scriptScope.getJavascriptLookup().lookupJavascriptClassBinding(methodName, userArgumentsSize);
                    ...
                    if (classBinding == null) {
                        ...
                        classBinding = scriptScope.getJavascriptLookup().lookupJavascriptClassBinding(methodName, userArgumentsSize + 1);
        ...
        if (localFunction != null) {
            ...
            semanticScope.setUsesInstanceMethod();
            semanticScope.putDecoration(userCallLocalNode, new ThisJavascriptMethod(thisMethod));
            ...
            semanticScope.putDecoration(userCallLocalNode, new StandardJavascriptMethod(importedMethod));
            ...
            semanticScope.putDecoration(userCallLocalNode, new StandardJavascriptClassBinding(classBinding));
            ...
            semanticScope.putDecoration(userCallLocalNode, new StandardJavascriptInstanceBinding(instanceBinding));
        ...
        if (semanticScope.getScriptScope().getCompilerSettings().areRegexesEnabled() == CompilerSettings.RegexEnabled.FALSE) {
            throw userRegexNode.createError(
                new IllegalStateException(
                    "Regexes are disabled. Set [script.javascript.regex.enabled] to [true] "
                        + "in elasticsearch.yaml to allow them. Be careful though, regexes break out of Javascript's protection against deep "
    ...
    public void visitLambda(ELambda userLambdaNode, SemanticScope semanticScope) {
        ...
        boolean deferredTarget = targetType == null || targetType.targetType() == def.class;
        ...
        if (deferredTarget) {
            ...
            for (String type : canonicalTypeNameParameters) {
                if (type == null) {
                    ...
                    Class<?> typeParameter = scriptScope.getJavascriptLookup().canonicalTypeNameToType(type);
            ...
            interfaceMethod = scriptScope.getJavascriptLookup().lookupFunctionalInterfaceJavascriptMethod(targetType.targetType());
            ...
            for (int i = 0; i < canonicalTypeNameParameters.size(); i++) {
                ...
                if (paramType == null) {
                    ...
                    Class<?> typeParameter = scriptScope.getJavascriptLookup().canonicalTypeNameToType(paramType);
        ...
        if (deferredTarget) {
            valueType = targetType == null ? String.class : def.class;
            semanticScope.putDecoration(
                userLambdaNode,
    ...
    public void visitSymbol(ESymbol userSymbolNode, SemanticScope semanticScope) {
        ...
        if (staticType != null) {
                ...
                throw userSymbolNode.createError(
                    new IllegalArgumentException(
                        "invalid assignment: "
                            ...
                            + JavascriptLookupUtility.typeToCanonicalTypeName(staticType)
                            + "]"
                    )
                );
            }
            ...
            if (read == false) {
                throw userSymbolNode.createError(
                    new IllegalArgumentException(
                        "not a statement: " + "static type [" + JavascriptLookupUtility.typeToCanonicalTypeName(staticType) + "] not used"
                    )
                );
            }
    ...
    public void visitDot(EDot userDotNode, SemanticScope semanticScope) {
        ...
        if (semanticScope.hasDecoration(userPrefixNode, PartialCanonicalTypeName.class)) {
            ...
            String canonicalTypeName = semanticScope.getDecoration(userPrefixNode, PartialCanonicalTypeName.class)
                ...
                + "."
                + index;
            Class<?> staticType = scriptScope.getJavascriptLookup().canonicalTypeNameToType(canonicalTypeName);
            ...
            if (prefixStaticType != null) {
                ...
                staticType = scriptScope.getJavascriptLookup().canonicalTypeNameToType(staticCanonicalTypeName);
            }
            ...
            if (staticType != null) {
                ...
                if (prefixValueType != null && prefixValueType.valueType().isArray()) {
                    ...
                    JavascriptField field = semanticScope.getScriptScope()
                        .getJavascriptLookup()
                        .lookupJavascriptField(prefixType, isStatic, index);

                    if (field == null) {
                        JavascriptMethod getter;
                        JavascriptMethod setter;

                        getter = scriptScope.getJavascriptLookup()
                            .lookupJavascriptMethod(
                                prefixType,
                                isStatic,
                        ...
                        setter = scriptScope.getJavascriptLookup()
                            .lookupJavascriptMethod(
                                prefixType,
                                isStatic,
                        ...
                        if (getter != null || setter != null) {
                            ...
                            if (getter != null) {
                                semanticScope.putDecoration(userDotNode, new GetterJavascriptMethod(getter));
                            }

                            if (setter != null) {
                                semanticScope.putDecoration(userDotNode, new SetterJavascriptMethod(setter));
                            }

                            semanticScope.setCondition(userDotNode, Shortcut.class);
                        } else if (isStatic == false) {
                            if (Map.class.isAssignableFrom(prefixValueType.valueType())) {
                                getter = scriptScope.getJavascriptLookup().lookupJavascriptMethod(prefixType, false, "get", 1);
                                setter = scriptScope.getJavascriptLookup().lookupJavascriptMethod(prefixType, false, "put", 2);
                            ...
                            if (List.class.isAssignableFrom(prefixType)) {
                                ...
                                getter = scriptScope.getJavascriptLookup().lookupJavascriptMethod(prefixType, false, "get", 1);
                                setter = scriptScope.getJavascriptLookup().lookupJavascriptMethod(prefixType, false, "set", 2);
                        ...
                        semanticScope.putDecoration(userDotNode, new StandardJavascriptField(field));
    ...
    public void visitBrace(EBrace userBraceNode, SemanticScope semanticScope) {
        ...
        if (prefixValueType.isArray()) {
            ...
            String canonicalClassName = JavascriptLookupUtility.typeToCanonicalTypeName(prefixValueType);

            JavascriptMethod getter = semanticScope.getScriptScope()
                .getJavascriptLookup()
                .lookupJavascriptMethod(prefixValueType, false, "get", 1);
            JavascriptMethod setter = semanticScope.getScriptScope()
                .getJavascriptLookup()
                .lookupJavascriptMethod(prefixValueType, false, "put", 2);
            ...
            if ((read == false || getter != null) && (write == false || setter != null)) {
                ...
                if (getter != null) {
                    semanticScope.putDecoration(userBraceNode, new GetterJavascriptMethod(getter));
                }

                if (setter != null) {
                    semanticScope.putDecoration(userBraceNode, new SetterJavascriptMethod(setter));
                }
            } else {
                throw userBraceNode.createError(
            ...
            JavascriptMethod setter = semanticScope.getScriptScope()
                .getJavascriptLookup()
                .lookupJavascriptMethod(prefixValueType, false, "set", 2);
            ...
            throw userBraceNode.createError(
                new IllegalArgumentException(
                    "Illegal array access on type " + "[" + JavascriptLookupUtility.typeToCanonicalTypeName(prefixValueType) + "]."
                )
            );
        }
    ...
    public void visitCall(ECall userCallNode, SemanticScope semanticScope) {
        ...
        JavascriptMethod method = null;

        if (prefixValueType != null) {
            Class<?> type = prefixValueType.valueType();
            JavascriptLookup lookup = semanticScope.getScriptScope().getJavascriptLookup();

            if (prefixValueType.valueType() == def.class) {
                dynamic = true;
            } else {
                method = lookup.lookupJavascriptMethod(type, false, methodName, userArgumentsSize);

                if (method == null) {
                    JavascriptClass pc = lookup.lookupJavascriptClass(type);
                    dynamic = pc != null
                        && pc.annotations.containsKey(DynamicTypeAnnotation.class)
                        && lookup.lookupJavascriptSubClassesMethod(type, methodName, userArgumentsSize) != null;
            ...
            method = semanticScope.getScriptScope()
                .getJavascriptLookup()
                .lookupJavascriptMethod(prefixStaticType.staticType(), true, methodName, userArgumentsSize);
        ...
        if (dynamic) {
            ...
            semanticScope.putDecoration(userCallNode, new StandardJavascriptMethod(method));
