...
import static org.elasticsearch.javascript.WriterConstants.NEEDS_PARAMETER_METHOD_TYPE;

/**
 * Information about the interface being implemented by the javascript script.
 */
public class ScriptClassInfo {
    ...
    public ScriptClassInfo(JavascriptLookup javascriptLookup, Class<?> baseClass) {
                    ...
                    getReturns.add(
                        definitionTypeForClass(
                            ...
                            componentType -> "["
                                ...
                                + "]. Javascript can only support getters with return types that are "
                                + "whitelisted."
                        )
                    );
        ...
        executeMethodReturnType = definitionTypeForClass(
            ...
            componentType -> "Javascript can only implement execute methods returning a whitelisted type but ["
                + baseClass.getName()
                + "#execute] returns ["
                + componentType.getName()
        ...
        for (int arg = 0; arg < types.length; arg++) {
            arguments.add(methodArgument(javascriptLookup, types[arg], argumentNamesConstant[arg]));
        }
    ...
    /**
     * The interface that the Javascript script should implement.
     */
    ...
    /**
     * The Javascript {@link Class} or the return type of the {@code execute} method. This is used to generate the appropriate
     * return bytecode.
     */
    ...
    /**
     * Javascript {@link Class}s and names of the arguments to the {@code execute} method. The names are exposed to the Javascript
     * script.
     */
    ...
    /**
     * The {@code uses$varName} methods that must be implemented by Javascript to complete implementing the interface.
     */
    ...
    /**
     * The {@code getVarName} methods that must be implemented by Javascript to complete implementing the interface.
     */
    ...
    /**
     * Javascript {@link Class}es and name of the argument to the {@code execute} method.
     */
    public record MethodArgument(Class<?> clazz, String name) {}

    private static MethodArgument methodArgument(JavascriptLookup javascriptLookup, Class<?> clazz, String argName) {
        Class<?> defClass = definitionTypeForClass(
            ...
            componentType -> "["
                ...
                + "] is of unknown type ["
                + componentType.getName()
                + ". Javascript interfaces can only accept arguments that are of whitelisted types."
        );
    ...
    private static Class<?> definitionTypeForClass(
        ...
        type = JavascriptLookupUtility.javaTypeToType(type);
        ...
        if (componentType != def.class && javascriptLookup.lookupJavascriptClass(componentType) == null) {
