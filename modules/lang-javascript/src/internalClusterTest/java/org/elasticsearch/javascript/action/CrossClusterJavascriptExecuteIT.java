...
/**
 * Tests the JavascriptExecuteAction against a local cluster with one remote cluster configured.
 * Execute action tests are run against both the local cluster and the remote cluster.
 */
public class CrossClusterJavascriptExecuteIT extends AbstractMultiClustersTestCase {
    ...
    protected Collection<Class<? extends Plugin>> nodePlugins(String clusterAlias) {
        List<Class<? extends Plugin>> plugs = Arrays.asList(JavascriptPlugin.class);
        return Stream.concat(super.nodePlugins(clusterAlias).stream(), plugs.stream()).collect(Collectors.toList());
    }

    public void testJavascriptExecuteAgainstLocalCluster() throws Exception {
        setupTwoClusters();
        ...
        JavascriptExecuteAction.Request.ContextSetup contextSetup = createContextSetup(LOCAL_INDEX);
        JavascriptExecuteAction.Request request = new JavascriptExecuteAction.Request(script, context.name, contextSetup);

        ActionFuture<JavascriptExecuteAction.Response> actionFuture = client(LOCAL_CLUSTER).admin()
            .cluster()
            .execute(JavascriptExecuteAction.INSTANCE, request);

        JavascriptExecuteAction.Response response = actionFuture.actionGet();
     ...
     */
    public void testJavascriptExecuteAsCrossClusterAction() throws Exception {
        setupTwoClusters();
        ...
        JavascriptExecuteAction.Request.ContextSetup contextSetup = createContextSetup(REMOTE_CLUSTER + ":" + REMOTE_INDEX);
        JavascriptExecuteAction.Request request = new JavascriptExecuteAction.Request(script, context.name, contextSetup);
    ...
    private static JavascriptExecuteAction.Request.ContextSetup createContextSetup(String index) {
        ...
        JavascriptExecuteAction.Request.ContextSetup contextSetup = new JavascriptExecuteAction.Request.ContextSetup(index, doc, query);
